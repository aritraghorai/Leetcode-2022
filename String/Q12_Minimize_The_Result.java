...
/*
 !Name: Aritra Ghorai
 !Date:16/04/2022
 ?Program Details:2232. Minimize Result by Adding Parentheses to Expression
 *https://leetcode.com/problems/minimize-result-by-adding-parentheses-to-expression/
   */
public class Q12_Minimize_The_Result {
    public String minimizeResult(String expression) {
        ...
        for (int i = n1 - 1; i >= 0; i--) {
            for (int j = 0; j < n2; j++) {
                ...
                if (sum < min) {
                    min = sum;
                    ans = s1.substring(0, i) + "(" + s1.substring(i) + "+" + s2.substring(0, j + 1) + ")"
                            + s2.substring(j + 1);
                }
            }
        }
